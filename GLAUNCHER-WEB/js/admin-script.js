document.addEventListener('DOMContentLoaded', () => {
    ...
    const gateErrorMessage = document.getElementById('gate-error-message');
    ...
    adminKeySubmit.addEventListener('click', () => {
        ...
        if (key) {
            localStorage.setItem('glauncher_admin_key', key);
            ADMIN_KEY = key;
            initializeAdminPanel();
        } else {
            gateErrorMessage.textContent = 'Por favor, ingresa una clave.';
        }
    });

    adminKeyInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            adminKeySubmit.click();
        }
    });

    function checkAdminKey() {
        const storedKey = localStorage.getItem('glauncher_admin_key');
        if (storedKey) {
            ADMIN_KEY = storedKey;
            initializeAdminPanel();
        } else {
    ...
    async function loadUsers() {
        ...
        try { // Añadimos el header de autorización
    ...
    async function sendPromptToGemini(prompt, sectionContext = null) {
        ...
        try {
            const response = await fetch(`${BACKEND_URL}/api/gemini-chat`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include', // Gemini también debería estar protegido
    ...
    // Iniciar la comprobación de la clave de administrador al cargar la página
    checkAdminKey();
});